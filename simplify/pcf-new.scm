  ...
  (define (the-gcd p1 p2)
	...
	(let ((v1 (poly/horner p1 args)) (v2 (poly/horner p2 args)))
	  (cond ((or (not (exact? v1)) (not (exact? v2)))
		 (if heuristic-gcd-wallp? (write-line 'gcd-lose-inexact!))
		 :one)
		((base/one? (base/gcd v1 v2))
		 (if heuristic-gcd-wallp? (write-line 'gcd-win!))
		 :one)
		(else
		 (if heuristic-gcd-wallp? (write-line 'gcd-grunt!))
		 (poly/gcd p1 p2)))))))
  the-gcd)
...
(define (unordered-poly-hash a modulus)
  (let ((arity (poly/check-same-arity (car a) (cdr a))))
	...
	(modulo (* (numerator v) (denominator v))
		modulus)))))

;;; Support for heuristic stuff and memoizer
